...
Eigen::Vector3d toGlobal(double latitude0, double longitude0, double altitude0, const double x, const double y, const double z){
  Eigen::Vector3d local_ecef;
  try {
    GeographicLib::Geocentric earth(GeographicLib::Constants::WGS84_a(),
            GeographicLib::Constants::WGS84_f());
    ...
    ROS_FATAL("Global2Local::toGlobal - unable to perform transformation");
  }
  ...
  Eigen::Vector3d global(lat, lon, alt);
  return global;
}
...
bool Geographiclib_local2global_service::callback(graupner_serial::LatLong::Request& request, 
												  graupner_serial::LatLong::Response& response)
{
	double height = 0;
	//_earth_local->Reset(request.lattitude0, request.longitude0, height);
	...
	for(int i = 0; i<request.longitude.size(); i++){
		//global_point = toGlobal(request.latitude0, request.longitude0, request.altitude0, request.latitude[i], request.longitude[i], height, true);
		//_earth_local->Forward(request.lattitude[i], request.longitude[i], 0, x, y, z);
		response.x.push_back(global_point.x());
		response.y.push_back(global_point.y());
	}
	return true;
}	
