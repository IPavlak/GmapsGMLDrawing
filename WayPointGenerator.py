...
'''
!!!!!!!!!!!!!!!!!!!!!!!  IMPORTANT
In this file trajectory refers only to path points (x, y),
it does NOT incorporate information about the time at which UAV should be at 
that location or the appropriate velocities (or accelerations) at that location
!!!!!!!!!!!!!!!!!!!!!!!
'''

class GenerateTrajectory:
    def __init__(self, coords):
        self.building_coords = coords
...
'''
def generate_trajectory_path(x, y, dist, res, res_building_points, height, min_height, res_height):
...
def generate_trajectory_path_single_z(traj_x, traj_y, traj_z, traj_yaw):
    h = traj_z[0]
    single_z_traj_x = [traj_x[0]]; single_z_traj_y = [traj_y[0]]
    single_z_traj_yaw = [traj_yaw[0]]
    i = 1
    while traj_z[i] == h:
        single_z_traj_x.append(traj_x[i])
        single_z_traj_y.append(traj_y[i])
        single_z_traj_yaw.append(traj_yaw[i])
        i += 1

    return single_z_traj_x, single_z_traj_y, single_z_traj_yaw
